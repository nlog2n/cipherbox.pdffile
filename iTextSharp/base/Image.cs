        ...
        */
        ...
        public static Image GetInstance(Uri url, bool handleIncorrectImage) {
            ...
            try {
                ...
                throw new IOException("unknown.image.format: " + url.ToString());
            } finally {
                if (istr != null) {
                    istr.Close();
                }
        ...
        */
        ...
        public static Image GetInstance(int width, int height, bool reverseBits, int typeCCITT, int parameters, byte[] data, int[] transparency) {
            if (transparency != null && transparency.Length != 2)
                throw new DocumentException("BadElement: transparency.length.must.be.equal.to.2.with CCITT images");
        ...
        public static Image GetInstance(int width, int height, int components, int bpc, byte[] data, int[] transparency) {
            if (transparency != null && transparency.Length != components * 2)
                throw new DocumentException("BadElement: transparency.length.must.be.equal.to.componentes.2");
