...
namespace CipherBox.Pdf.Parser {
    ...
    public class PdfImageObject {
        ...
        private void DecodeImageBytes() {
            ...
            if (pngColorType < 0) {
                if (bpc != 8)
                    throw new InvalidPdfException(string.Format("Unsupported: the.color.depth.{0}.is.not.supported", bpc));
                if (PdfName.DEVICECMYK.Equals(colorspace)) {
                    ...
                    if (!PdfName.ICCBASED.Equals(tyca))
                        throw new InvalidPdfException(string.Format("Unsupported: the.color.space.{0}.is.not.supported", colorspace));
                    PRStream pr = (PRStream)ca.GetDirectObject(1);
                    int n = pr.GetAsNumber(PdfName.N).IntValue;
                    if (n != 4) {
                        throw new InvalidPdfException(string.Format("Unsupported: N.value.{0}.is.not.supported", n));
                    }
                    icc = PdfReader.GetStreamBytes(pr);
                }
                else
                ...
                wr.AddField(new TiffWriter.FieldAscii(TIFFConstants.TIFFTAG_SOFTWARE, SoftwareVersion.Version));
