...
namespace CipherBox.Pdf.Parser
{
    ...
    {
        public const String DEFAULTOPERATOR = "DefaultOperator"; // Default operator
        ...
        private void DisplayXObject(PdfName xobjectName)
        {
            ...
            {
                throw new InvalidOperationException(string.Format("XObject {0} is not a stream", xobjectName));
            }
        }
        ...
        private class ProcessGraphicsStateResource : IContentOperator
        {
            public void Invoke(PdfContentStreamProcessor processor, PdfLiteral oper, List<PdfObject> operands)
            {
                ...
                if (extGState == null)
                    throw new ArgumentException(string.Format("resources.do.not.contain.extgstate.entry.unable.to.process.oper {0}", oper));
                PdfDictionary gsDic = extGState.GetAsDict(dictionaryName);
                if (gsDic == null)
                    throw new ArgumentException(dictionaryName + " is.an.unknown.graphics.state.dictionary");
